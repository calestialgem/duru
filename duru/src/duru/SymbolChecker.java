...
public final class SymbolChecker {
  ...
  private CheckedStatement checkStatement(Node.Statement node) {
    return switch (node) {
      ...
      case Node.Affect affect ->
        throw Diagnostic.unimplemented(affect.location());
    };
  }
  ...
  private CheckedExpression checkExpression(Node.Expression node) {
    switch (node) {
      case Node.UnaryOperator op ->
        throw Diagnostic.unimplemented(op.location());
      case Node.BinaryOperator op ->
        throw Diagnostic.unimplemented(op.location());
