    ...
    Node.Statement node)
  {
    return switch (node) {
      ...
      case Node.If s -> {
        ...
        Semantic.Expression          condition    =
          check_expression(inner, s.condition());
    ...
    Node.Expression node)
  {
    return switch (node) {
      case Node.LogicalOr(var l, var r) -> {
        Semantic.Expression left_operand  = check_expression(scope, l);
        Semantic.Expression right_operand = check_expression(scope, r);
        ...
        else if (right_operand instanceof Semantic.KnownNumeric right) {
          if (right.numeric() == 1) {
            yield new Semantic.KnownNumber(1);
          }
          yield new Semantic.NotEqualTo(
