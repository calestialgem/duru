...
public sealed interface Node {
  ...
  record Entrypoint(Statement body) implements Declaration {
    ...
    public int first(List<Token> tokens) {
      return body.first(tokens) - 1;
    }

    @Override
    public int last(List<Token> tokens) {
      return body.last(tokens);
    }
  ...
  sealed interface Definition extends Declaration {
    ...
    @Override
    default Token representative(List<Token> tokens) {
      return identifier();
    }
  }
  ...
  record Parameter(Token.Identifier identifier, boolean reference)
    implements Node
  {
    @Override
    public int first(List<Token> tokens) {
    ...
    @Override
    public int last(List<Token> tokens) {
      return tokens.indexOf(identifier) + (reference ? 1 : 0);
    }
  }
  ...
  record GlobalVar(
    Optional<Token.Public> modifier,
    Token.Identifier identifier,
    Optional<Expression> initialValue) implements Definition
  {
    @Override
    public int first(List<Token> tokens) {
      if (modifier.isPresent()) {
    ...
    public int last(List<Token> tokens) {
      if (initialValue.isPresent()) {
        return initialValue.get().last(tokens) + 1;
      }
      return tokens.indexOf(identifier) + 1;
    }
  }
  ...
  record Block(int first, List<Statement> innerStatements)
    implements Statement
  {
    ...
    public int last(List<Token> tokens) {
      if (!innerStatements.isEmpty()) {
        return innerStatements.getLast().last(tokens) + 1;
      }
      return first + 1;
    }
  }
    ...
    Optional<Statement> falseBranch) implements Statement
  {
    ...
    public int first(List<Token> tokens) {
      if (!variables.isEmpty()) {
        return variables.get(0).first(tokens) - 1;
      }
    ...
    public int last(List<Token> tokens) {
      if (falseBranch.isPresent()) {
        return falseBranch.get().last(tokens);
      }
      return trueBranch.last(tokens);
    }
  }
  ...
  record While(
    ...
    Statement loop,
    Optional<Statement> zeroBranch) implements Statement
  {
    @Override
    public int first(List<Token> tokens) {
      if (label.isPresent()) {
    ...
    public int last(List<Token> tokens) {
      if (zeroBranch.isPresent()) {
        return zeroBranch.get().last(tokens);
      }
  ...
  record LocalVar(
    Token.Identifier identifier,
    Optional<Expression> initialValue) implements Statement
  {
    @Override
    public int first(List<Token> tokens) {
      return tokens.indexOf(identifier) - 1;
  ...
  sealed interface Mutate extends Affect {
    ...
    default int first(List<Token> tokens) {
      return target().first(tokens);
    }

    @Override
    default int last(List<Token> tokens) {
      return target().last(tokens) + 1;
    }
  }
  ...
  sealed interface BinaryOperation extends Expression {
    ...
    @Override
    default int first(List<Token> tokens) {
      return leftOperand().first(tokens);
    }

    @Override
    default int last(List<Token> tokens) {
      return rightOperand().last(tokens);
    }
  }
  ...
  record LogicalOr(Precedence11 leftOperand, Precedence11 rightOperand)
  ...
  record LogicalAnd(Precedence10 leftOperand, Precedence10 rightOperand)
  ...
  record EqualTo(Precedence09 leftOperand, Precedence09 rightOperand)
    implements Precedence09, BinaryOperation
  {}

  /** Expression that yields one when the left operand is not equal to the right
   * operand, and zero otherwise. */
  record NotEqualTo(Precedence09 leftOperand, Precedence09 rightOperand)
    implements Precedence09, BinaryOperation
  {}
   ...
   * otherwise. */
  record StrictlyEqualTo(Precedence09 leftOperand, Precedence09 rightOperand)
    implements Precedence09, BinaryOperation
  {}
   ...
   * operand, and zero otherwise. */
  record LessThan(Precedence08 leftOperand, Precedence08 rightOperand)
  ...
  record LessThanOrEqualTo(Precedence08 leftOperand, Precedence08 rightOperand)
    implements Precedence08, BinaryOperation
  {}

  /** Expression that yields one when the left operand is greater than the right
   * operand, and zero otherwise. */
  record GreaterThan(Precedence08 leftOperand, Precedence08 rightOperand)
    implements Precedence08, BinaryOperation
  {}
  ...
  record GreaterThanOrEqualTo(
    Precedence08 leftOperand,
    Precedence08 rightOperand) implements Precedence08, BinaryOperation
  {}
  ...
  record BitwiseOr(Precedence07 leftOperand, Precedence07 rightOperand)
  ...
  record BitwiseXor(Precedence06 leftOperand, Precedence06 rightOperand)
  ...
  record BitwiseAnd(Precedence05 leftOperand, Precedence05 rightOperand)
   ...
   * integers. */
  record LeftShift(Precedence04 leftOperand, Precedence04 rightOperand)
   ...
   * operand when the operands are taken as 53-bit signed two's complement
   * integers. */
  record RightShift(Precedence04 leftOperand, Precedence04 rightOperand)
    implements Precedence04, BinaryOperation
  {}
  ...
  record Addition(Precedence03 leftOperand, Precedence03 rightOperand)
    implements Precedence03, BinaryOperation
  {}

  /** Expression that yields the subtraction of its operands. */
  record Subtraction(Precedence03 leftOperand, Precedence03 rightOperand)
    implements Precedence03, BinaryOperation
  {}
  ...
  record Multiplication(Precedence02 leftOperand, Precedence02 rightOperand)
    implements Precedence02, BinaryOperation
  {}

  /** Expression that yields the division of its operands. */
  record Division(Precedence02 leftOperand, Precedence02 rightOperand)
    implements Precedence02, BinaryOperation
  {}

  /** Expression that yields the left operand in mod right operand. */
  record Modulus(Precedence02 leftOperand, Precedence02 rightOperand)
    implements Precedence02, BinaryOperation
  {}
  ...
  record Grouping(Expression grouped) implements Precedence00 {
    @Override
    public int first(List<Token> tokens) {
      return grouped.first(tokens) - 1;
    }

    @Override
    public int last(List<Token> tokens) {
      return grouped.last(tokens) + 1;
    }
  }
  ...
  record SymbolAccess(Mention accessed) implements SymbolBased {
    @Override
    public int first(List<Token> tokens) {
      return accessed.first(tokens);
    }

    @Override
    public int last(List<Token> tokens) {
      return accessed.last(tokens);
    }
  }
    ...
    implements SymbolBased
  {
    @Override
    public int first(List<Token> tokens) {
      return called.first(tokens);
    }

    @Override
    public int last(List<Token> tokens) {
      if (!arguments.isEmpty()) {
        return arguments.getLast().last(tokens) + 1;
      }
    ...
    implements Precedence00
  {
    @Override
    public int first(List<Token> tokens) {
      return object.first(tokens);
    }

    @Override
    public int last(List<Token> tokens) {
      return tokens.indexOf(member);
    }
  }
  ...
  record MemberCall(
    Precedence00 firstArgument,
    Token.Identifier called,
    List<Expression> remainingArguments) implements Precedence00
  {
    @Override
    public int first(List<Token> tokens) {
      return firstArgument.first(tokens);
    }

    @Override
    public int last(List<Token> tokens) {
      if (!remainingArguments.isEmpty()) {
        return remainingArguments.getLast().last(tokens) + 1;
      }
      return firstArgument.last(tokens) + 4;
    }
  }

  /** Expression that directly denotes a compile-time known number value. */
  record NaturalConstant(Token.NaturalConstant token) implements Precedence00 {
    @Override
    public int first(List<Token> tokens) {
  ...
  /** Expression that directly denotes a compile-time known number value. */
  record RealConstant(Token.RealConstant token) implements Precedence00 {
    @Override
    public int first(List<Token> tokens) {
      return tokens.indexOf(token);
    }
  ...
  record StringConstant(Token.StringConstant token) implements Precedence00 {
    @Override
    public int first(List<Token> tokens) {
      return tokens.indexOf(token);
    }
